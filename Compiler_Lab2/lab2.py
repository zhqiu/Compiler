...
class Node:
	def __init__(self, type, p, children=None, leaf=None):
		self.type = type
		if children:
...
def SynAnaTree(root, indent):
	...
	if root == None:
		return
	print(' '*indent, end='')
	if type(root) == list and len(root) == 2 and root[0] in ['ID', 'TYPE', 'INT', 'FLOAT']:
...
def p_exp_RELOP(p):
	'Exp : Exp RELOP Exp'
	p[0] = Node('Exp', p, [p[1], 'RELOP', p[3]], p[0])
...
"""
print('SymbolTable:')
for item in SemAnalysis.getSymbolTable():  # item is a global variable, a structure or a function
	print(item)
"""
