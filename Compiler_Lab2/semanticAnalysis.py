...
class SemAnalysis(object):
	...
	def procCompSt(self, compst, func_var_list, func_name, return_type):
		...
		self.checkDefAssign(symboltable, func_name)
	...
	def procStmt(self, stmt, symboltable, func_name, return_type):
		...
		if len(stmt.children) == 1:    # Stmt : CompSt
			...
			self.procExp(stmt.children[0], symboltable, func_name)

		elif len(stmt.children) == 3:  # Stmt : RETURN Exp SEMI
			# check the function's return type
			exp_tp = self.procExp(stmt.children[1], symboltable, func_name)
			...
			self.procExp(stmt.children[2], symboltable, func_name)
	...
	# symboltable contains symbols defined in this function
	def procExp(self, exp, symboltable, func_name):
		...
		if len(exp.children) == 1:  
			...
			exp_tp = self.procExp(exp.children[1], symboltable, func_name)
			...
			if exp.children[1] == 'DOT':                               #	  |	Exp DOT ID
				...
				if type(exp.children[0].children[0]) != list:  # it's a Node, should be processed as a Exp !!
					exp_type = self.procExp(exp.children[0], symboltable, func_name)
				...
				elif if_defined == 0 and func_not_define:
					print('Error type 2 at Line %d: Undefined function \"%s\"' %(exp.lineno, function_name))		

				if args_wrong and match_func_name != None:
				...
				return self.procExp(exp.children[1])

			else:
				exp1, exp2 = exp.children[0], exp.children[2] # type: Node

				exp1_tp = self.procExp(exp1, symboltable, func_name)         # check two components
				exp2_tp = self.procExp(exp2, symboltable, func_name)
				...
				if exp.children[1] in ['AND', 'OR', 'RELOP']:
			...
			if exp.children[1] == 'LP' and exp.children[3] == 'RP':   # Exp : ID LP Args RP
				...
				function_name = exp.children[0][1]
				args_list = []
				for s in self.procArgs(exp.children[2], [], symboltable, func_name):
					if s in ['INT', 'FLOAT']:
				...
				if if_defined == 1:   # it's a variable defined in this function
					...
					return_type, func_var_list = self.getFuncArgsList(function_name) 

					if func_var_list == None:
						print('Error type 2 at Line %d: Undefined function \"%s\"' %(exp.lineno, function_name))
						return None

					elif args_list != func_var_list:
						s1, s2 = '', ''
				...
				if type(exp1.children[0]) == list:  # it's a ID ! need to check if the variable has been defined
					...
					exp1_tp = self.procExp(exp1, symboltable, func_name)
	...
	def procArgs(self, args, al, symboltable, func_name):
		# Args : Exp COMMA Args
		# 	   | Exp
		if len(args.children) == 1:
			al.append(self.procExp(args.children[0], symboltable, func_name))

		elif len(args.children) == 3:
			al.append(self.procExp(args.children[0], symboltable, func_name))
			al = self.procArgs(args.children[2], al, symboltable, func_name)
	...
	def checkStructDef(self, struct_def):
		for item in self.SymbolTable:
			if item[0] == 'global struct' and item[1][0] == struct_def[0]:
				return True
		return False
